...
int main(int argc, char **argv)
{
	...
	char int_inp_flag, byte_inp_flag, terse_output_flag;

	int_inp_flag = byte_inp_flag = terse_output_flag = 0;
	NAME = argv[0];

	while ((opt = getopt(argc, argv, "i:b:t")) != -1) {
		switch (opt) {
		case 'i':
			inp_int = atoi(optarg);
			int_inp_flag = 1;
	...
	printf("%d\t%s\n",
	       //inp_int, BYTE_ARR_WIDTH / 8, byte_rep(inp_int)
	       inp_int, byte_rep(inp_int)
	    );
	return 0;
}
